...
from eval_utils import pfm_to_pil, load_image
import numpy as np
...
def process(device, model, model_type, image, input_size, target_size, optimize, use_camera, image_name, epsilon, targeted_attack=False):
    """
    ...
    if "openvino" in model_type:
        ...
        gt = load_image(image_name, mode='depth')
        ...
        if targeted_attack:
            ...
            # binary object mask
            mask = cv2.imread("semantic_annotations/" + image_name.split("/")[1], cv2.IMREAD_GRAYSCALE)
            mask = to_tensor(mask)
            mask = torch.where(mask < 0.5, torch.zeros_like(mask), torch.ones_like(mask))
            # Convert mask to bool tensor
            bool_mask = mask.bool()
            loss = loss_fn(prediction, c * mask + disp_tensor * (1 - mask), bool_mask)
        else:
            loss = loss_fn(prediction, disp_tensor, valid_mask)
        model.zero_grad()
        loss.backward()
        data_grad = sample.grad.data
        perturbed_sample = fgsm_attack(sample, epsilon, data_grad)
        perturbed_prediction = model.forward(perturbed_sample)
        
        # prediction = prediction.squeeze().detach().numpy()
        perturbed_prediction = (
            torch.nn.functional.interpolate(
                perturbed_prediction.unsqueeze(1),
                size=target_size[::-1],
                mode="bicubic",
                align_corners=False,
            )
    ...
    # python3 evaluate.py -gt ../../MiDaS/depth_annotations/ -pred ../../MiDaS/dpt_swin2_tiny_targeted/eps_0.25/
...
def run(input_path, output_path, model_path, model_type="dpt_beit_large_512", optimize=False, side=False, height=None,
    ...
    for epsilon in epsilons:
        perturbed_output_path = output_path + f'eps_{epsilon}'

        # create output folder
        # if perturbed_output_path is not None:
            # os.makedirs(perturbed_output_path, exist_ok=True)

        print("Start processing")
        if input_path is not None:
            if perturbed_output_path is None:
                print("Warning: No output path specified. Images will be processed but not shown or stored anywhere.")
            for index, image_name in enumerate(image_names):
                print("  Processing {} ({}/{})".format(image_name, index + 1, num_images))

                # filename = os.path.join(
                #         perturbed_output_path, os.path.splitext(os.path.basename(image_name))[0]
                # )
                ...
                # FGSM attack
                perturbed_prediction = process(device, model, model_type, image, (net_w, net_h), original_image_rgb.shape[1::-1],
                                    optimize, False, image_name, epsilon, targeted_attack=True)
                ...
                if perturbed_output_path is not None:
                    ...
                    #     utils.write_depth(filename, perturbed_prediction, grayscale, bits=2)
                    # else:
                        # original_image_bgr = np.flip(original_image_rgb, 2)
                        # content = create_side_by_side(original_image_bgr*255, prediction, grayscale)
                        # perturbed_content = create_side_by_side(original_image_bgr*255, perturbed_prediction, grayscale)
                        # cv2.imwrite(filename + ".png", content)
                        # cv2.imwrite(filename + ".png", perturbed_content)

                    original_image_bgr = np.flip(original_image_rgb, 2)
                    perturbed_content = create_side_by_side(original_image_bgr*255, perturbed_prediction, grayscale)
                    cv2.imwrite(filename + ".png", perturbed_content)
                    # utils.write_pfm(filename + ".pfm", prediction.astype(np.float32))
                    utils.write_pfm(filename + ".pfm", perturbed_prediction.astype(np.float32))
